...
class MongoDB:
    ...
    def create_database(self, database_name, collection_name, dct):
        ...
        log.debug(
            "create_database() - database_name:{0} collection_name:{1}".format(
    ...
    def databases(self, showAll=False):
        ...
        if not showAll:
            lstDBNames = [
                dbName for dbName in lstDBNames if dbName not in self.lstDBIgnore
            ]
        for dbName in lstDBNames:
            ...
            if not showAll:
                lstCollNames = [
                    colName
                    for colName in lstCollNames
                    if colName not in self.lstCollIgnore
                ]
    ...
    def collection_drop(self, dbName, collName):
        """ drop a collection from a database """
        log.debug(
            "collection_drop() - dbName:{0} collName:{1}".format(dbName, collName)
        )
        db = self._conn[str(dbName)]
    ...
    def collection_count(self, dbName, collName):
        """ return number of documents in collection """
        log.debug(
            "collection_count() - dbName:{0} collName:{1}".format(dbName, collName)
        )
        db = self._conn[str(dbName)]
        co = db[str(collName)]
        return co.count()
    ...
    def find(self, dbName, collName, query=None, fields=None, skip=0, limit=0):
        """ drop a collection from a database """
        log.debug(
            "find() - dbName:{0} collName:{1} query:{2} fields:{3} skip:{4} limit:{5}".format(
                dbName, collName, query, fields, skip, limit
            )
        )
        cur = None
        spec = {}
    ...
    def create_index(self, ddName, collName, index):
        """  create an index """
        log.debug(
            "create_index() - dbName:{0} collName:{1} index:{2}".format(
                dbName, collName, index
            )
        )
        obj = None
        db = self._conn[str(dbName)]
...
if __name__ == "__main__":
    ...
    try:
        ...
        # find one
        dct = db.find_one(
            dbName, dbColl, query={"text": "This is a test doc for bulk insert 1"}
        )
        print(dct)
    finally:
        if db:
