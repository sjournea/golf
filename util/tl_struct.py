...
class StructPack(StructImpl):
  ...
  def cbVarString(self, args):
    try:
      s = self.GetNextArg()
      self.binData += struct.pack('%cH%ds' % (self.endian_char, args), args, s[:args])
    except struct.error, err:
  ...
  def cbVarJSON(self, args):
    try:
      ...
      self.binData += struct.pack('%cH%ds' % (self.endian_char, length), length, json_string)
    except struct.error, err:
...
class StructUnpack(StructImpl):
  ...
  def _unpackData(self, name, fmt, byte_size):
    try:
      value, = struct.unpack('%c%s' % (self.endian_char, fmt), self.binData[self.index:self.index+byte_size])
  ...
  def cbVarString(self, args):
    try:
      length, = struct.unpack('%cH' % self.endian_char, self.binData[self.index:self.index+2])
      self.index += 2
      s, = struct.unpack('%ds' % length, self.binData[self.index:self.index+length])
      self.index += length
      s = str(s).replace('\0','')
  ...
  def cbVarBinary(self, args):
    try:
      data, = struct.unpack('%c%ds' % (self.endian_char, args), self.binData[self.index:self.index+args])
      self.index += args
