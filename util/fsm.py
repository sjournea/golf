...
class FSM_Event:
    ...
    def executeAction(self, fsm):
        ...
        if nextState is None:
            log.warn(
                'Event "%s" not valid in state "%s" ... ignoring'
                % (self.name, fsm.fsm_get_state())
            )
        elif nextState == "*":
            log.debug(
                'Event "%s" ignored in state "%s"' % (self.name, fsm.fsm_get_state())
            )
        else:
...
class FSM:
  ...
  """
    ...
    """
        log.debug(
            "fsm_set_state() - newState:%s oldState:%s evt:%s"
            % (newStateName, self._curState.name, evt)
        )
        ...
        def __init__(self, name=None):
            ...
            lstStates = [
                FSM_State(
                    ST_GREEN,
                    self._stGreen,
                    fnEnter=self._greenEnter,
                    fnExit=self._greenExit,
                ),
            ...
            lstActions = [
                FSM_Event(EVT_GO, {ST_RED: ST_GREEN}, fnBefore=self._goBefore),
            ...
            # FSM base class
            super().__init__(
                lstStates, lstActions, ST_RED, fnEnterState=self._enterState, name=name
            )
