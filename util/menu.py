...
class MenuItem:
    ...
    def __init__(self, cmd, command, desc, func=None):
        ...
        self.func = func
        if self.cmd != "":
            assert self.cmd not in MenuItem.lstCmds, 'cmd "%s %s"  already defined' % (
                self.cmd,
                self.command,
            )
    ...
    def __init__(self, cmd_file=None, menuSize=50):
        ...
        self._lstBuiltInMenuItems = [
            ...
            MenuItem(
                "",
                "<any command> --repeat",
                "Adding --repeat will cause this command to be repeated",
            ),
    ...
    def runMenu(self):
        while True:
            try:
                if self.showCmds:
                    self.updateHeader()
                    print(f"\nCommands available:\t{self.header}")
                ...
                if not self.bRepeatCommand:
                    ...
                    if len(lst) > 1:
                        ...
                        if len(lst) % 2 == 0:
                            raise Exception(
                                "Illegal command -- mismatched double quotes"
                            )
                        self.lstCmd = []
