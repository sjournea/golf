...
class SqlBestBallTeam(SqlGolfTeam):
    ...
    def get_scorecard(self, index):
        ...
        for i, score in enumerate(self._score[9:]):
            line += " {:>3}".format(score_string(index, score))
        line += " {:>4} {:>4}".format(
            score_string(index, self._in), score_string(index, self._total)
        )
        dct["line"] = line
        return dct
...
class SqlGameBestBall(SqlGameNet):
...
"""
    ...
    game_options = {
        "teams": {
            "default": [(0, 1), (2, 3)],
            "type": "tuple[2][2]",
            "desc": "Set teams for best ball match.",
        }
    }
    ...
    def setup(self, **kwargs):
        ...
        self._players = [
            NetPlayer(self, result, 0) for result in self.golf_round.results
        ]
        # create teams
        self.team_list = [
            SqlBestBallTeam(self, [self._players[i1], self._players[i2]])
            for (i1, i2) in self.teams
        ]
    ...
    def update(self):
        ...
        super(SqlGameBestBall, self).update()
        # now do team updates
    ...
    def getScorecard(self, **kwargs):
        ...
        self.dctScorecard["players"] = [
            team.get_scorecard(index) for index, team in enumerate(self.team_list)
        ]
    ...
    def getLeaderboard(self, **kwargs):
        ...
        for n, team in enumerate(self.team_list):
            ...
            if team._total > 0 or (team._total == 0 and n == 0):
                line += team.status
            dct = {"team": team, "line": line, "status": team.status}
            board.append(dct)
        self.dctLeaderboard["leaderboard"] = board
        self.dctLeaderboard["hdr"] = "{:<10}{}".format(
            "Match", "Final" if self.final else "Thru {}".format(self.thru)
        )
