...
class SkinsPlayer(GamePlayer):
    def __init__(self, game, result, min_handicap):
        super(SkinsPlayer, self).__init__(game, result)
...
"""
    game_options = {
        "use_carryover": {
            "default": True,
            "type": "bool",
            "desc": "If use_carryover is set then skins not won will carry to the next hole",
        }
    }
    ...
    def setup(self, **kwargs):
        ...
        self._players = [
            SkinsPlayer(self, result, min_handicap)
    ...
    def getLeaderboard(self, **kwargs):
        board = []
        scores = sorted(
            self._players, key=lambda score: score.dct_skins["total"], reverse=True
        )
        pos = 1
        prev_total = None
        for sc in scores:
            score_dct = {"player": sc.player, "total": sc.dct_skins["total"]}
