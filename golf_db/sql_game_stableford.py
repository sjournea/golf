...
(especially classic Stableford) using their full handicaps."""
    ...
    game_options = {
        "stableford_type": {
            "default": "British",
            "type": "choice",
            "choices": ("Modified", "Classic", "British", "Spanish"),
            "desc": "Modified, Classic, British or Spanish",
        },
        "jokers": {
            "default": "(9,18)",
            "type": "tuple[2]",
            "desc": "Set joker holes for each player. One in front 9 and one in back 9",
        },
    ...
    def validate(self):
        ...
        if self.stableford_type not in self.dct_scoring:
            raise GolfException(
                "stableford_type {} not supported.".format(self.stableford_type)
            )
        if self.stableford_type == "Spanish":
            ...
            if len(self.jokers) != len(self.scores):
                raise GolfException(
                    "stableford_type Spanish needs a joker set for each player."
                )
            for joker in self.jokers:
                if len(joker) != 2:
                    raise GolfException(
                        "stableford_type Spanish jokers must have 2 values."
                    )
                if joker[0] not in (1, 2, 3, 4, 5, 6, 7, 8, 9):
                    raise GolfException(
                        "stableford_type Spanish joker[0] must be in 1-9."
                    )
                if joker[1] not in (10, 11, 12, 13, 14, 15, 16, 17, 18):
                    raise GolfException(
                        "stableford_type Spanish joker[1] must be in 10-18."
                    )
    ...
    def setup(self, **kwargs):
        ...
        self._players = [
            StablefordPlayer(self, result) for result in self.golf_round.results
        ]
    ...
    def update(self):
        ...
        for pl, result in zip(self._players, self.golf_round.results):
            ...
            for n, score in enumerate(result.scores):
                ...
                pl.dct_points["holes"][n] = self._calc_score(
                    net_score - self.golf_round.course.holes[n].par
                )
                if pl._jokers:
    ...
    def getLeaderboard(self, **kwargs):
        ...
        prev_total = None
        for sc in scores:
            score_dct = {"player": sc.player, "total": sc.dct_points["total"]}
