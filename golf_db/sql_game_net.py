...
"""
    game_options = {
        "use_full_net": {
            "default": False,
            "type": "bool",
            "desc": "Use full net instead of relative to lowest handicap.",
        }
    }
    ...
    def setup(self, **kwargs):
        ...
        self._players = [
            player_class(self, result, min_handicap)
    ...
    def getScorecard(self, **kwargs):
        ...
        for n, sc in enumerate(self._players):
            ...
            line = "{:<3} {:>2}".format(
                sc.player.getInitials(), sc.result.course_handicap
            )
            for net, bump in zip(sc.dct_net["holes"][:9], sc._bumps[:9]):
                nets = "{}{}".format(bump * "*", net if net and net > 0 else "")
                line += " {:>3}".format(nets)
