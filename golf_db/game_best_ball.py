...
"""
  ...
  def start(self):
    ...
    min_handicap = min([gs.course_handicap*handicap_multiplier for gs in self.scores])
    for pl in self.scores:
      pl._net = [None for _ in range(len(self.golf_round.course.holes))]
      pl._bumps = self.golf_round.course.calcBumps(pl.course_handicap*handicap_multiplier - min_handicap)
    # create teams
    self.team_list = [BestBallTeam([self.scores[i1], self.scores[i2]]) for (i1,i2) in self.teams]
  ...
  def getLeaderboard(self, **kwargs):
    if not self.final:
      ...
      self.dctLeaderboard['hdr'] = '{:<10}{}'.format('Match', 'Final' if self.final else 'Thru {}'.format(thru))
      if self.final:
      ...
      if score is None:
        ...
        for pl in self.scores:
          if pl._bumps[n] > 0:
            ...
            bumps.append(dct)
            bump_line.append('{}{}'.format(pl.player.nick_name, '({})'.format(dct['bumps']) if dct['bumps'] > 1 else ''))
