    ...
    def load_game_options(self, **kwargs):
        ...
        def set_value(dct, value):
            if dct["type"] == "int":
                ...
                if not lst:
                    raise GolfException(
                        "No matching choice with {} in {}".format(value, dct["choices"])
                    )
                if len(lst) > 1:
                    raise GolfException(
                        "Multiple matches with choice {} in {}".format(
                            value, dct["choices"]
                        )
                    )
                value = lst[0]
                ...
                raise GolfException(
                    'option type "{}" not supported'.format(dct["type"])
                )
...
class GamePlayer(object):
    ...
    def calc_bumps(self, min_handicap):
        return self.game.golf_round.course.calcBumps(
            self.result.course_handicap - min_handicap
        )
