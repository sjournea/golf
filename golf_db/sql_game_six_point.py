...
class SixPointPlayer(NetPlayer):
    def __init__(self, game, result, min_handicap):
        super(SixPointPlayer, self).__init__(game, result, min_handicap)
        self.dct_points = self._init_dict()


class SqlGameSixPoint(SqlGameNet):
    """Six point golf game."""
...
"""
    ...
    def validate(self):
        if len(self._players) != 3:
            raise GolfException(
                "{} game must have 3 players, {} found.".format(
                    self.TITLE, len(self.scores)
                )
            )
    ...
    def setup(self, **kwargs):
        ...
        min_handicap = min(
            [result.course_handicap for result in self.golf_round.results]
        )
        self._players = [
            SixPointPlayer(self, result, min_handicap)
            for result in self.golf_round.results
        ]
    ...
    def update(self):
        ...
        super(SqlGameSixPoint, self).update()
    ...
    def getLeaderboard(self, **kwargs):
        board = []
        scores = sorted(
            self._players, key=lambda pl: pl.dct_points["total"], reverse=True
        )
        pos = 1
