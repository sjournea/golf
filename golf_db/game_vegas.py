...
class VegasTeam(GolfTeam):
    ...
    def calculate_score(self, index):
        ...
        for player in self.players:
            self.multiplier[index] = max(
                (self.course.holes[index].par - player.gross[index]) + 1,
                self.multiplier[index],
                1,
            )
    ...
    def update_points(self, index, other_team):
        self.points[index] = (
            max(other_team.score[index] - self.score[index], 0) * self.multiplier[index]
        )
    ...
    def __str__(self):
        return "{} - points_total:{} handicap:{} total_bumps:{} bumps:{}".format(
            self.name, self.points_total, self.handicap, self.total_bumps, self.bumps
        )
...
"""
    ...
    def validate(self):
        if len(self.scores) != 4:
            raise GolfException(
                "Vegas game must have 4 players, {} found.".format(len(self.scores))
            )
        if len(self.teams) != 2:
    ...
    def start(self):
        # create teams
        self.team_list = [
            VegasTeam([self.scores[i1], self.scores[i2]])
            for n, (i1, i2) in enumerate(self.teams)
        ]
    ...
    def getLeaderboard(self, **kwargs):
        ...
        prev_total = None
        for team in teams:
            score_dct = {"team": team, "total": team.points_total}
