...
class RewardsPlayer(GrossPlayer):
    ...
    def update_rewards(self):
        self.dct_rewards["out"] = self.dct_rewards["holes"][:9].count(
            "P"
        ) + self.dct_rewards["holes"][:9].count("B")
        self.dct_rewards["in"] = self.dct_rewards["holes"][9:].count(
            "P"
        ) + self.dct_rewards["holes"][9:].count("B")
...
"""
    ...
    def getLeaderboard(self, **kwargs):
        ...
        board = []
        scores = sorted(
            self._players, key=lambda score: score.dct_rewards["total"], reverse=True
        )
        pos = 1
        prev_total = None
        for score in scores:
            score_dct = {"player": score.player, "total": score.dct_rewards["total"]}
