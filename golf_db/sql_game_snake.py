...
"""
    game_options = {
        "snake_type": {
            ...
            "default": "Hold",
            "type": "choice",
            "desc": "Points will lose every 3 putt. Hold will only lose when you already have the snake, always pays on 9 and 18.",
        }
    }
    ...
    def setup(self, **kwargs):
        """Start the game."""
        self._players = [
            SnakePlayer(self, result) for result in self.golf_round.results
        ]
                ...
                if len(lst_losers) > 1:
                    ...
                    if len(lst_losers) > 1:
                        if hole_num in self.game._game_data:
                            ...
                            lst_losers = [
                                tup
                                for tup in lst_losers
                                if tup[0].player.nick_name == loser
                            ]
                        if len(lst_losers) > 1:
                            dct = {
                                "hole_num": hole_num,
                                ...
                                "key": "closest_3_putt",
                                "msg": "Which 3 putt player had the closest 1st putt on hole {}?".format(
                                    hole_num
                                ),
                                "game": self,
                            }
        ...
        for n, putt in enumerate(self._players[0].dct_points["holes"]):
            if putt is None:
                ...
                if self.snake_type == "Hold":
                    if self._has_snake:
                        line = "{} has the snake.".format(
                            self._has_snake.player.nick_name
                        )
                    else:
                        line = "snake is free."
