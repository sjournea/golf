...
class GreeniePlayer(GamePlayer):
    def __init__(self, game, result):
        ...
        self.dct_greens = (
            [None for _ in range(len(self.game.golf_round.course.holes))],
        )
...
"""
    game_options = {
        "double_birdie": {
            "default": False,
            "type": "bool",
            "desc": "Birdies are worth double points.",
        },
        "carry_over": {
            "default": True,
            "type": "bool",
            "desc": "If nobody wins a par 3 then carries over to next par 3.",
        },
        "last_par_3_carry": {
            "default": True,
            "type": "bool",
            "desc": "If nobody wins last par 3 then carry over to next hole and on green in regulation quailifies.",
        },
    ...
    def setup(self, **kwargs):
        ...
        self._holes = [
            hole.num for hole in self.golf_round.course.get_holes_with_par(3)
        ]
        if self.last_par_3_carry:
            self._last_par_3 = self._holes[-1]
            self._holes += [
                hole.num for hole in self.golf_round.course.holes[self._last_par_3 :]
            ]
        self._players = [
            GreeniePlayer(self, result) for result in self.golf_round.results
        ]
        self._carry = 0
        self._next_hole = 0
        ...
        for index in range(self.thru):
            ...
            if hole_num in dct_greens:
                ...
                if len(lst_winners) > 1:
                    if hole_num in self.game._game_data and self.game._game_data[
                        hole_num
                    ].get("qualified"):
                        qualified = self.game._game_data[hole_num]["qualified"]
                        lst_winners = [
                            w
                            for w in lst_winners
                            if str(w[0].player.nick_name) == qualified
                        ]
                    else:
                        dct = {
                            "hole_num": hole_num,
                            ...
                            "key": "qualified",
                            "msg": "Which player was closest to the pin on hole {}?".format(
                                hole_num
                            ),
                            "game": self,
                        }
                ...
                if len(lst_winners) == 1:
                    ...
                    if score.putts < 3:
                        ...
                        if self.wager:
                            winner.dct_money["holes"][index] = winner.dct_points[
                                "holes"
    ...
    def getLeaderboard(self, **kwargs):
        ...
        if sort_type == "money" and self.wager:
            ...
            scores = sorted(
                self._players, key=lambda score: score.dct_money["total"], reverse=True
            )
            sort_by = "money"
        else:
            self.dctLeaderboard["hdr"] = "Pos Name  Points Thru"
            scores = sorted(
                self._players, key=lambda score: score.dct_points["total"], reverse=True
            )
            sort_by = "total"
        pos = 1
        prev_total = None
    ...
    def getStatus(self, **kwargs):
        ...
        if self._next_hole is None:
            ...
            if self._next_hole in self._holes:
                self.dctStatus["par"] = self.golf_round.course.holes[
                    self._next_hole
                ].par
