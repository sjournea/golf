...
class MatchPlayer(NetPlayer):
    def __init__(self, game, result, min_handicap):
        super(MatchPlayer, self).__init__(game, result, min_handicap)
...
"""
    ...
    def validate(self):
        if len(self._players) != 2:
            raise GolfException(
                "Match game must have 2 players, {} found.".format(len(self._players))
            )
    ...
    def setup(self, **kwargs):
        ...
        super(SqlGameMatch, self).setup(**kwargs)
        self.winner = None
    ...
    def update(self):
        """Start the match game."""
        super(SqlGameMatch, self).update()
    ...
    def getLeaderboard(self, **kwargs):
        ...
        for n, pl in enumerate(self._players):
            line = "{:<10}".format(pl.player.nick_name)
            if pl.total > 0 or (pl.total == 0 and n == 0):
                line += pl.status
            dct = {"player": pl, "line": line, "status": pl.status}
            board.append(dct)
