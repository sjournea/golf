...
class RoundLeaderboard(GolfView):
    ...
    def _update_controls(self):
        ...
        if self.game.game.game_type in (
            ...
            for widthPos, hdr, alignment, width in zip(
                widthPositions, headers, alignments, widths
            ):
                lblHdr = ui.Label(
                    text=hdr,
                    alignment=alignment,
                    center=(widthPos, hdrHeight),
                    font=("<system-bold>", 20),
                    width=width,
                )
            ...
            playerHeights = [
                hdrHeight * (n + 2)
                for n in range(len(self.dctLeaderboard["leaderboard"]))
            ]
            ...
            for n, dct in enumerate(self.dctLeaderboard["leaderboard"]):
                ...
                for widthPos, key, alignment, width in zip(
                    widthPositions, keys, alignments, widths
                ):
                    if key == "player":
            ...
            for n, dct in enumerate(self.dctLeaderboard["leaderboard"]):
                ...
                for widthPos, key, alignment, width in zip(
                    ...
                    if key == "thru" and n == 0:
                        ...
                        if dct["team"]._total > 0 or (
                            dct["team"]._total == 0 and n == 0
                        ):
                            text = dct["status"]
                    if text:
                        lbl = ui.Label(
