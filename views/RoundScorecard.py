...
class RoundScorecard(GolfView):
    ...
    def did_load(self):
        ...
        width = 50
        # create ScrollView
        self.cardView = ui.ScrollView(
            x=self.width * 0.15,
            y=self.height * 0.10,
            width=self.width * 0.85,
            height=self.height * 0.30,
        )
        ...
        lstHoles = (
            [str(hole) for hole in range(1, 10)]
            + ["Out"]
            + [str(hole) for hole in range(10, 19)]
            + ["In", "Total", "ESC"]
        )
        lstWidths = 22 * [50]
        ...
        for widthPos, text, width in zip(widthOffsets, lstHoles, lstWidths):
            ...
            font = (
                "<system-bold>" if text in ("In", "Out", "Total", "ESC") else "<system>"
            )
            lbl = ui.Label(
                text=text,
    ...
    def _update_controls(self):
        ...
        if self.game.game.game_type in (
            "gross",
            "net",
            "putts",
            "stableford",
            ...
            for n in range(4):
                ...
                if n < len(self.dctScorecard["players"]):
                    ...
                    if self.game.game.game_type == "net":
                        self.lstPlayerLabels[n].text += " {:>3}".format(
                            self.golf_round.results[n].course_handicap
                        )
    ...
    def activate(self):
        ...
        self.golf_round = (
            session.query(Round)
            .filter(Round.round_id == self._mainView._round_id)
            .one()
        )
