...
class RoundScores(GolfView):
    ...
    def did_load(self):
        ...
        self.lblHole = ui.Label(
            text="Hole Desc",
            center=(self.width * 0.5, self.height * 0.05),
    ...
    def activate(self):
        ...
        for n, result in enumerate(self.golf_round.results):
            ...
            self.players.append(
                PlayerForm(result.result_id, self.btnGross[n], self.btnPutts[n])
            )
    ...
    def _get(self, session):
        ...
        for player in self.players:
            score = (
                session.query(Score)
                .filter(
                    Score.result_id == player.result_id, Score.num == self._hole_num
                )
                .first()
            )
    ...
    def set_gross(self, sender):
        ...
        title = "Hole {} Par {} - Gross".format(
            self._current_hole.num, self._current_hole.par
        )
