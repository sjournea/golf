...
class GolfStablefordGameTest(unittest.TestCase):
  ...
  def test_game_add_score(self):
    ...
    self.assertEqual(g.scores[3]._points[0], 2)
    g.addScore(1, [3,4,5,6])
    self.assertEqual(g.scores[0]._points[1], 8)
    self.assertEqual(g.scores[1]._points[1], 5)
    self.assertEqual(g.scores[2]._points[1], 2)
    self.assertEqual(g.scores[3]._points[1], 0)
    g.addScore(2, [3,3,4,4])
    self.assertEqual(g.scores[0]._points[2], 2)
    self.assertEqual(g.scores[1]._points[2], 2)
    self.assertEqual(g.scores[2]._points[2], 0)
    self.assertEqual(g.scores[3]._points[2], 0)
