...
class TestGolfStablefordGame:
    ...
    def test_game_init(self):
        ...
        assert g.stableford_type == "Classic"
        assert g.jokers is None

        g = StablefordGame(self.gr, self.gr.scores, stableford_type="British")
        assert g.stableford_type == "British"
        assert g.jokers is None

        with pytest.raises(GolfException):
            StablefordGame(self.gr, self.gr.scores, stableford_type="Spanish")

        with pytest.raises(GolfException):
            StablefordGame(
                self.gr,
                self.gr.scores,
                stableford_type="Spanish",
                jokers=((1, 10), (2, 11), (3, 12)),
            )
        ...
        g = StablefordGame(
            ...
            self.gr.scores,
            stableford_type="Spanish",
            jokers=((1, 10), (2, 11), (3, 12), (4, 13)),
        )
        assert g.stableford_type == "Spanish"
        assert g.jokers == ((1, 10), (2, 11), (3, 12), (4, 13))
    ...
    def test_game_wager(self):
        ...
        assert g.stableford_type == "Classic"
        assert g._wager == 0.5
        assert g.total_payout == wager * num_players * num_holes
    ...
    def test_game_add_score(self):
        ...
        assert g.scores[0].dct_points["holes"][0] == 2
        assert g.scores[1].dct_points["holes"][0] == 2
        assert g.scores[2].dct_points["holes"][0] == 2
        assert g.scores[3].dct_points["holes"][0] == 2

        g.addScore(1, [3, 4, 5, 6])

        assert g.scores[0].dct_points["holes"][1] == 8
        assert g.scores[1].dct_points["holes"][1] == 5
        assert g.scores[2].dct_points["holes"][1] == 2
        assert g.scores[3].dct_points["holes"][1] == 0

        g.addScore(2, [3, 3, 4, 4])

        assert g.scores[0].dct_points["holes"][2] == 2
        assert g.scores[1].dct_points["holes"][2] == 2
        assert g.scores[2].dct_points["holes"][2] == 0
        assert g.scores[3].dct_points["holes"][2] == 0
